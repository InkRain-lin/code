...
void showTask() {
    for (int i = 0; i < AllTaskCount; i++) {
        printf("| %2d | %-38s|  %2dh%2dm  |  %2d/%2d   |  ",i + 1, AllTaskArr[i].title, AllTaskArr[i].time / 60, AllTaskArr[i].time % 60, AllTaskArr[i].deadline[1], AllTaskArr[i].deadline[2]);  //  输出任务
        //  任务是否完成
...
void changeTaskScreen() { //maininput用3表示
    ...
    while (1){
        ...
        gotoxy(41,8);
        scanf("%4d", &AllTaskArr[index - 1].time);
        gotoxy(41,9);
